...
菜单里的按钮不限制点击人——谁点就按谁的身份执行，能不能执行由服务端判定。
...
| `百万美金菜单` | 显示公开菜单按钮 |
| `百万美金转账` | 先选收款人（按房间序号，如「2.名字」），再选金额；转账不会导致退出 |
| `百万美金退出` | 退出指定人物槽位并收回保证金；3 人局需要选择槽位 |
| `百万美金准备` / `百万美金取消准备` | 有活动槽位的玩家全部准备后立即结算；转账会清空所有人的准备，需要重新准备 |
...
最后一名玩家选完角色后，机器人会立即发送匿名中央牌堆图片：公开角色显示正面，随机隐藏的
一张显示卡背。谈判公告下直接提供一套所有玩家共用的“转账 / 退出本轮 / 准备 / 使用威胁牌 /
查看状态”按钮，无需再次打开菜单。按钮点击后的发送者身份决定实际操作者；选择收款目标、
金额或人物槽位时，才会进入仅本人可用的 HMAC 按钮。

### 卡图

- `百万美金帮助` 只发送**规则卡**图片（`docs/sources/rule-cards/rule-card.jpg`），不再追加文字消息；
- **开局**会发送本轮赃物牌正面图片，并在正文列出地点、赃款、保证金和奖励角色效果；
- **选角完成**会发送匿名中央牌堆图片，公开角色为正面，隐藏角色为卡背；
