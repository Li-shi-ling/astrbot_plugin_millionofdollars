...
菜单会根据发送者身份和当前阶段，只显示此刻可用的操作。玩家专属操作只能由菜单请求者点击；
首领在谈判满 60 秒后会看到“强制抢劫”，选角按钮丢失时可用“重新获取选角”。
转账、退出和威胁牌等二级菜单都提供“返回菜单”。按钮点击后仍需**发送**才会生效。

## 玩法指令

所有按钮都等价于一条文本指令；秘密动作（选角、转账金额、退出确认、告密人指定）
只提供 `百万美金操作 <HMAC令牌>` 按钮，没有明文回退。

| 指令 | 说明 |
| --- | --- |
| `百万美金创建` | 在当前群创建房间，发送者成为首领 |
| `百万美金加入` | 加入房间，仅大厅阶段可用，最多 8 人 |
| `百万美金退出房间` | 大厅阶段退出房间；最后一人退出时房间自动关闭，首领退出时首领转交 |
| `百万美金关闭` | 关闭当前房间（`关闭房间`/`结束`/`解散` 同义），仅首领或 AstrBot 管理员可用 |
| `百万美金开始` | 首领开局，人数需 3～8 人 |
| `百万美金选角` | 选角阶段重新生成自己的选角按钮，并使自己之前的选角按钮失效 |
| `百万美金状态` | 查看人数、阶段、回合、赃物牌、公开角色计数与各玩家现金 |
| `百万美金菜单` | 显示公开菜单按钮 |
| `百万美金转账` | 先选收款人，再选金额；转账不会导致退出 |
| `百万美金退出` | 退出指定人物槽位并收回保证金；3 人局需要选择槽位 |
...
旧的 `/百万美金 创建` 写法仍由解析层兼容，但菜单按钮和使用说明统一生成 `百万美金创建`。
QQ Markdown 状态消息会突出关键字段；
同一次回复包含多条消息时，每条消息使用独立递增的 `msg_seq`。所有指令都会显式终止事件，
不会继续触发 AstrBot 的默认 LLM 回复。

同一群的游戏操作会按顺序处理并发送，避免多人同时加入时人数消息前后倒置。开局会先发送
本轮赃物牌图片，再发送包含赃款、保证金和奖励角色效果的 Markdown。多人选角按钮按玩家分行
合并：每行仍只有对应玩家能够点击，5 人局只需一条选角键盘，6～8 人局拆成两条，避免超过
QQ 对单次事件的被动回复次数限制。

### 卡图

- `百万美金帮助` 只发送**规则卡**图片（`docs/sources/rule-cards/rule-card.jpg`），不再追加文字消息；
- **开局**会发送本轮赃物牌正面图片，并在正文列出地点、赃款、保证金和奖励角色效果；
